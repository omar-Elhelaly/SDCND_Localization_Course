...
vector<int> NN(PointCloudT::Ptr target, PointCloudT::Ptr source, Eigen::Matrix4d initTransform, double dist){
	...
	// TODO loop through each transformed source point and using the KDtree find the transformed source point's nearest target point.
	// Append the nearest point to associaitons. Simply push that target point index into vector<int> associations.
	int idx = 0;
	for (PointT point : transformSource->points) {// POINT `pcl::PointXYZ` (input point from transformed source)
